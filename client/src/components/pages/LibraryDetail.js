...
export default class LibraryDetail extends Component {
  ...
  leaveLibrary() {
    api
      .deleteMember(this.state.member._id, this.props.match.params.libraryId)
      .then(response => {
  ...
  deleteLibrary() {
    api
      .deleteLibrary(this.props.match.params.libraryId)
      .then(response => {
  ...
  handleDeleteMember(indexToRemove) {
    this.setState({
      allmembers: this.state.allmembers.filter(
        (member, i) => i !== indexToRemove
      )
    });
  }
      ...
      <div className="LibraryDetail">
          ...
          <div className="libraryCard">
            ...
            <Card>
              ...
              <CardBody>
                ...
                {this.state.member && (
                  <Button
                    href={`/send-invitation/${this.state.library._id}`}
                ...
                {this.state.member && this.state.member.role === "admin" && (
                  <Button
                    onClick={this.toggleAlertDeleteLibrary}
                ...
                <Alert
                  color="info"
                  isOpen={this.state.showAlertDeleteLibrary}
                  toggle={this.toggleAlertDeleteLibrary}
                >
                ...
                {this.state.member && this.state.member.role === "member" && (
                  <Button
                    onClick={this.toggleAlertLeaveLibrary}
                    className="btn btn-info"
                  >
                    Leave
                  </Button>
                ...
                <Alert
                  color="info"
                  isOpen={this.state.showAlertLeaveLibrary}
                  toggle={this.toggleAlertLeaveLibrary}
                >
                  Are you sure you want to leave this library? <br />
                  <Button
                    onClick={e => this.leaveLibrary(e)}
                    className="btn btn-danger"
                  >
                    Leave!
                  </Button>
        ...
        <div className="memberList">
          ...
          {this.state.allmembers &&
            this.state.allmembers.map((members, i) => (
              <div key={members._id}>
                <Card>
                  <Row>
                    <Col xs="3">
